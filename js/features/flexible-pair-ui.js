...
function rosterFromState(){return(loadState()?.roster||[]).map((entry,index)=>({id:String(entry?.id||`ui:${index}`),name:String(entry?.name||entry?.teamName||'').trim(),teamName:String(entry?.teamName||'').trim()})).filter(entry=>entry.name)}
