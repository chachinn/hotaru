...
function patchFlexiblePair(){
  ...
  const mode=smart.querySelector('#team-mode')?.value;if(mode!=='lock2')return;
  const lock1=smart.querySelector('#team-lock-1')?.value||'',lock2=smart.querySelector('#team-lock-2')?.value||'';if(!lock1||!lock2||lock1===lock2)return;
  const host=smart.querySelector(':scope > .section');if(!host||host.querySelector('.team-results'))return;
  if(/Team review pending/i.test(host.textContent||''))return;
  ...
  const flexible=buildFlexiblePairTeams({roster,lockedNames:[lock1,lock2],allowUnowned,limit:12});if(!flexible.supported)return;
  if(!flexible.results.length){
    if(flexible.previewAvailable&&!allowUnowned)host.innerHTML='<div class="notice info"><strong>A flexible pair route exists, but it needs missing teammates.</strong><br>Turn on Allow unowned to preview mechanics-preserving Odette + Flins adaptations and see which support slots you are missing.</div>';
    return;
  }
