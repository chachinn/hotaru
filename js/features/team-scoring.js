...
export function teamInvestmentProfile(teamMembers=[],roster=[],{artifacts=[],weaponDataAvailable=false,artifactDataAvailable=false}={}){
  const map=rosterMap(roster),byLocation=artifactMap(artifacts),rows=(teamMembers||[]).map(name=>{
    const entry=map.get(key(name));if(!entry)return{name,score:-400,level:0};
    const artifactRows=byLocation.get(compactKey(entry.name))||byLocation.get(compactKey(entry.teamName))||[];
    return{name,level:clamp(entry?.level,1,90,1),score:characterInvestmentScore(entry,entry?.equippedWeapon||null,{artifacts:artifactRows,artifactDataAvailable,weaponDataAvailable})};
  });
  if(!rows.length)return{rows:[],peakInvestment:0,averageInvestment:0,minimumInvestment:0};
  const scores=rows.map(row=>row.score);
  return{rows,peakInvestment:Math.max(...scores),averageInvestment:Math.round(scores.reduce((sum,score)=>sum+score,0)/scores.length),minimumInvestment:Math.min(...scores)};
}
