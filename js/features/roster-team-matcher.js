...
export function matchReviewedTeams({roster=[],lockedNames=[],allowUnowned=false,limit=5}={}){
  ...
  const results=candidates.map(team=>{
    ...
    return{...team,members,missing,ownedCount,ownedComplete:missing.length===0,score:scoreReviewedTeam({...team,members},{roster:scoringRoster,ownedNames,lockedNames:locks})};
