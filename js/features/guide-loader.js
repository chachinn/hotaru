...
async function run(){enhanceExplorationGuide();const name=app?.querySelector('.detail-name')?.textContent?.trim();if(!name)return;const activeSection=app.querySelector('.segmented button.active')?.textContent?.trim().toLowerCase();if(activeSection==='build'&&app.querySelector('main .skeleton'))return;const catalog=await getCatalog();if(!catalog)return;const character=catalog.characters.find(c=>c.name===name);if(!character)return;if(lastName!==name){detailPromise=null;lastName=name}detailPromise=detailPromise||getCharacterDetail(character).catch(()=>null);const detail=await detailPromise;if(!detail)return;await enhanceReferenceGuide({app,catalog,detail,character,profile:inferBuildProfile(detail)});enhanceGuideTaxonomy(character).catch(()=>{})}
