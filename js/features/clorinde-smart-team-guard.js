...
document.addEventListener('click',event=>{
  ...
  if(locks.length!==2||!locks.some(name=>key(name)==='clorinde'))return;
  const other=locks.find(name=>key(name)!=='clorinde');if(!other)return;
  const compatibility=clorindeCompatibilityForCharacter(other);if(compatibility.adaptationAllowed)return;
  event.preventDefault();event.stopImmediatePropagation();
  const host=resultsHost(card);if(host)host.innerHTML=`<div class="notice info"><strong>No source-backed Clorinde pairing for ${escapeHtml(other)}.</strong><br>Hotaru checked the reviewed Clorinde compatibility audit and will not manufacture an adapted team for this pair. Choose another lock or use a sourced reviewed team.</div>`;
},{capture:true});
