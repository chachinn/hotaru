...
function normalizeTeams(value=[]){return(Array.isArray(value)?value:[]).filter(team=>Array.isArray(team?.members)&&team.members.length===4).slice(0,3).map(team=>({...team,members:team.members.map(name=>String(name||'').trim()).filter(Boolean)}))}
function normalizeVariant(variant={},index=0){
  const overrides={...(variant.overrides||{})};
  if('buildSummaryTeams'in overrides)overrides.buildSummaryTeams=normalizeTeams(overrides.buildSummaryTeams);
...
export const REMAINING_FIVE_STAR_BUILD_PROFILES=[...batchA,...batchB,...batchC,...batchD].map(normalizeProfile);
