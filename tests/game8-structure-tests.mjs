...
assert.equal(arleResolved.role,'On-field Pyro Main DPS','reviewed profile must override misleading healing-keyword inference');
assert.equal(arleResolved.roleGroup,'Main DPS');
assert.deepEqual(arleResolved.talentPriority,['attack','skill','burst'],'Game8-visible reviewed talent order must override generic inference');
...
for(const heading of ['Rating & Info','Strengths & Weaknesses','Best Artifacts','Best Weapons','Best Team Comps','Talents & How to Use','Materials'])assert.match(ui,new RegExp(heading.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
assert.match(ui,/reviewedBuildCards/,'reviewed characters must receive one Game8-style summary per legitimate build variant');
assert.match(ui,/data-reviewed-build-variant/);
assert.match(ui,/Goal Stats/);
