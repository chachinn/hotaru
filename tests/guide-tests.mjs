...
for(const file of ['js/data/character-reference.js','js/features/guide-engine.js','js/features/guide-ui.js','js/features/guide-loader.js'])execFileSync(process.execPath,['--check',path.join(root,file)],{stdio:'pipe'});
...
const loader=fs.readFileSync(path.join(root,'js/features/guide-loader.js'),'utf8');assert.match(loader,/observer\.observe\(app,\{childList:true\}\)/);assert.doesNotMatch(loader,/subtree:true/);assert.match(loader,/requestAnimationFrame/);
...
const index=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(index,/guide-loader\.js\?v=1\.0\.0/);assert.match(index,/guide-ui\.css\?v=1\.0\.0/);
const sw=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');assert.match(sw,/hotaru-shell-v7/);for(const file of ['character-reference.js','guide-engine.js','guide-ui.js','guide-loader.js'])assert.match(sw,new RegExp(file.replace('.','\\.')));
console.log('Hotaru deep character guide QA: all deterministic/static tests passed.');
