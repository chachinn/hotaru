...
const index=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(index,/guide-loader\.js\?v=1\.1\.0/);assert.match(index,/css\/guide-ui\.css\?v=1\.1\.0/);assert.match(index,/css\/exploration-ui\.css\?v=1\.0\.0/);
const sw=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');assert.match(sw,/hotaru-shell-v14/);for(const file of ['character-reference.js','map-registry.js','guide-engine.js','guide-ui.js','guide-loader.js','guide-taxonomy.js','exploration-ui.js','css/guide-ui.css','css/exploration-ui.css'])assert.match(sw,new RegExp(file.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
