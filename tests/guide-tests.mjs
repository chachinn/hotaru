...
for(const file of ['js/data/character-reference.js','js/features/guide-engine.js','js/features/guide-ui.js','js/features/guide-loader.js','js/features/exploration-ui.js'])execFileSync(process.execPath,['--check',path.join(root,file)],{stdio:'pipe'});
...
const loader=fs.readFileSync(path.join(root,'js/features/guide-loader.js'),'utf8');assert.match(loader,/observer\.observe\(app,\{childList:true\}\)/);assert.doesNotMatch(loader,/subtree:true/);assert.match(loader,/requestAnimationFrame/);assert.match(loader,/enhanceExplorationGuide/);
const ui=fs.readFileSync(path.join(root,'js/features/guide-ui.js'),'utf8');for(const text of ['Character profile','Talent priority','Constellations','Recommended weapons','Artifacts ranked','Goal stat values','Team comps','Ascension by level','Talent level-up materials','Exploration shortcuts'])assert.match(ui,new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
const explore=fs.readFileSync(path.join(root,'js/features/exploration-ui.js'),'utf8');for(const text of ['Shrines of Depths','Oculi','Teleport Waypoints','World Quests','Nod-Krai','Snezhnaya'])assert.match(explore,new RegExp(text));assert.doesNotMatch(explore,/innerHTML\s*\+=/);
const index=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(index,/guide-loader\.js\?v=1\.0\.0/);assert.match(index,/guide-ui\.css\?v=1\.0\.0/);assert.match(index,/exploration-ui\.css\?v=1\.0\.0/);
const sw=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');assert.match(sw,/hotaru-shell-v7/);for(const file of ['character-reference.js','guide-engine.js','guide-ui.js','guide-loader.js','exploration-ui.js','exploration-ui.css'])assert.match(sw,new RegExp(file.replace('.','\\.')));
