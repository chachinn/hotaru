...
assert.match(index,/smart-team-mobile-controller\.js\?v=1\.0\.6/,'index must keep the stable Current Abyss mobile controller request');
...
assert.match(sw,/const CACHE = 'hotaru-shell-v48'/,'service worker shell must be v48');
assert.match(sw,/PREVIOUS_CACHE = 'hotaru-shell-v47'/,'service worker must retain v47 as previous cache');
...
assert.match(sw,/smart-team-mobile-controller\.js\?v=1\.0\.6/,'service worker must keep the stable Current Abyss mobile controller request');
assert.match(sw,/smart-team-results-pagination\.js\?v=1\.2\.2&hotfix=52/,'v48 shell must retain the no-jump pagination cache entry');
assert.match(sw,/flexible-pair-ui\.js\?v=1\.0\.4/,'service worker must cache the ownership-safe flexible pair UI');
assert.match(updater,/RELEASE='v48'/,'PWA updater must target the recovery v48 shell');
