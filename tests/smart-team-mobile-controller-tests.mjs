...
assert.match(index,/roster-sections-team-filter\.css\?v=1\.2\.0/,'index must load the direct Team Need stylesheet revision');
assert.match(index,/roster-sections-team-filter\.js\?v=1\.2\.0/,'index must load the direct Team Need controller revision');
assert.match(index,/smart-team-mobile-controller\.js\?v=1\.0\.3/,'index must load the Current Abyss mobile controller revision');
assert.match(index,/flexible-pair-ui\.js\?v=1\.0\.2/,'index must load the ownership-safe flexible pair UI');
assert.match(sw,/const CACHE = 'hotaru-shell-v42'/,'service worker shell must advance to v42');
assert.match(sw,/PREVIOUS_CACHE = 'hotaru-shell-v41'/,'service worker must migrate from v41');
assert.match(sw,/roster-sections-team-filter\.css\?v=1\.2\.0/,'service worker must cache the direct Team Need stylesheet revision');
...
assert.match(sw,/smart-team-mobile-controller\.js\?v=1\.0\.3/,'service worker must cache the Current Abyss mobile controller revision');
assert.match(sw,/flexible-pair-ui\.js\?v=1\.0\.2/,'service worker must cache the updated flexible pair UI');
assert.match(updater,/RELEASE='v42'/,'PWA updater must request v42');
