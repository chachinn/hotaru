...
assert.match(ui,/hotaru\.team-utility-category\.v1/);
...
assert.match(ui,/id="hotaru-team-utility-category"/);
assert.match(ui,/Team Needs/);assert.match(ui,/>Need</);
assert.match(ui,/utilityOptionsForCategory/);
...
assert.match(css,/\.hotaru-team-utility-grid/);
assert.match(css,/\.hotaru-team-picker-ownership-field/,'ownership filter styling must remain after Team Needs expansion');
assert.match(css,/@media\(max-width:600px\)[\s\S]*\.hotaru-team-utility-grid\{grid-template-columns:1fr\}/,'Team Needs controls must stack on phone widths');
...
assert.match(index,/roster-sections-team-filter\.css\?v=1\.1\.0/);
