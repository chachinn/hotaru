...
assert.equal(planReviewedAbyssTeams({roster:sparse,allowUnowned:false}).results.length,0,'owned-only mode must not invent missing teammates');
const preview=planReviewedAbyssTeams({roster:sparse,allowUnowned:true,limit:2});
assert.ok(preview.results.length>=1);assert.ok(preview.results[0].missing.length>0);assert.ok(preview.results[0].ownedCount<8);
...
assert.ok(sw.includes('hotaru-shell-v26'));assert.ok(sw.includes('js/features/abyss-team-planner.js'));assert.ok(index.includes('app.js?v=1.12.0'));assert.ok(index.includes('style.css?v=1.8.0'));
