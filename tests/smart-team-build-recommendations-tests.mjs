...
const controller=fs.readFileSync('js/features/smart-team-v47-enhancements.js','utf8');
...
const style=fs.readFileSync('css/smart-team-v47.css','utf8');
const enhancements=style;
assert.match(controller,/mountSmartTeamRecommendations/,'v47 enhancement must mount the Teams / Recommendations view');
...
assert.match(controller,/Build next for this side/,'Abyss must recommend a level-up target separately for each side');
...
assert.match(enhancements,/@media\(max-width:700px\)[\s\S]*\.hotaru-map-head\{padding-top:15px!important;padding-bottom:8px!important/,'Map intro banner must be compact on phones');
