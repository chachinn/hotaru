...
const index=read('index.html'),sw=read('service-worker.js'),updater=read('js/pwa-update.js'),reviewedBootstrap=read('js/features/aloy-reviewed-bootstrap.js');
assert.ok(index.indexOf("navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'none'})")<index.indexOf('js/pwa-update.js?v=1.1.2'),'inline v48 rescue must begin before any external updater request can be trapped by a broken v47 worker');
assert.match(index,/hotaru\.pwa-reload\.v48/,'inline rescue and normal updater must share the v48 one-reload guard');
...
assert.equal((reviewedBootstrap.match(/registerReviewedTeams\(/g)||[]).length,1,'reviewed bootstrap must register the full catalog only once, not rebuild indexes per character');
...
assert.match(updater,/RELEASE='v48'/,'PWA reload marker must advance with the repaired shell');
...
assert.match(updater,/location\.reload\(\)/,'The page must reload once under the new worker so the frozen v47 bootstrap cannot remain active');
...
console.log('Hotaru v48 batched-bootstrap recovery + bounded startup Cache Storage + reviewed-team status regression QA passed.');
