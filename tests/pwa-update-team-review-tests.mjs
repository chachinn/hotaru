...
assert.match(sw,/const CACHE = 'hotaru-shell-v31'/);
assert.match(sw,/const PREVIOUS_CACHE = 'hotaru-shell-v30'/);
assert.match(sw,/js\/pwa-update\.js\?v=1\.0\.0/,'PWA updater must be available offline after the fresh shell is installed');
assert.match(updater,/RELEASE='v31'/,'PWA reload marker must advance with the shell');
